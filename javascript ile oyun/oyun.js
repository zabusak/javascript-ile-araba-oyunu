...
//setup
var c = document.createElement("canvas"); // oyun çizimi için oluşturuldu
c.width = window.innerWidth; // pencerenin eni
c.height = window.innerHeight; // pencerenin iç boyu
document.body.appendChild(c); // dokümanın body elementine appendChild kullanarak kanvası ekliyoruz
var ctx = c.getContext("2d");  // kanvasın içerisine 2 boyutlu çizim yapmak için kullanılır

var pts = [];
while (pts.length < 254) {
    while (pts.includes(val = Math.floor(Math.random() * 255)));
    pts.push(val); // her bir elemeandan bir tane olup rastgele bir arrayın içinde dizili değerler

} // y kooardinatları olarak kullanacağız
...
var noise = x => { //küçültme fonksiyonu
    x = x * 0.01 % 254;
...
// init
var bgcolor = "#0000ff"; // arka plan rengi turuncu
var forecolor = "#4a3f35"; //ön plan rengi acık gri
var linecolor = "#2f2519"; // çizgi rengi koyu gri
...
var player = new function () {
    ...
    this.drawInterface = function () {
        if (playing) {
            ...
            ctx.drawImage(this.startBtn, (c.width / 2) - 25, (c.height / 3) + 50, 50);
        }
    ...
    this.draw = function () {
        ...
        //rotation calc
        var angle = Math.atan2((p2 - 40) - this.y, (this.x + 5) - this.x);
        if (gnd && playing) {
...
function draw() {
    ...
    //bg
    ctx.fillStyle = bgcolor; // contextin rengi
    ctx.fillRect(0, 0, c.width, c.height); // arka planı siyah yapıp koordinatlarını belirledik
    // player
    player.draw();
    //ground
    ...
    ctx.beginPath();
    ctx.moveTo(offset, c.height - offset); //sıfırdan en köşeden başlıyor çizginin başladığı yer
    for (let i = offset; i < c.width - offset; ++i) { // for döngüsüyle 200 tane küçük çizgi çizdik
        ctx.lineTo(i, (c.height * .9) - noise(i + t) * yRatio); // enine çizdiğimiz çizgi
    }
    ...
    // animation
    requestAnimationFrame(draw); // fonk animasyondan çağırma
...
function checkBtnRelase(x,y){
    ...
    if (playing && x > 110 && y < 180 && y > ((c.height - 90) && y < (c.height - 20))) {
       k.ArrowRight = 0;
    }
